-- Mage Craft Database Schema - FINAL FIX
-- Generated: 2025-01-27
-- Purpose: Fixes the ambiguous column reference error
-- This version resolves all PostgreSQL naming conflicts
...
-- Helper function to add column if it doesn't exist (fixed naming conflict)
...
-- Function to create index only if it doesn't exist (fixed naming)
...
BEGIN
    IF NOT EXISTS (
        ...
        EXECUTE format('CREATE INDEX %I ON public.%I (%s)', index_name, target_table, columns);
    END IF;
END $$ LANGUAGE plpgsql;
...
-- Function to create policy only if it doesn't exist (fixed naming)
